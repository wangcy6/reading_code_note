...
#define ITEM_data(item)                                                        \
  ((char *)&((item)->data) + (item)->nkey + 1 + (item)->nsuffix +              \
   (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))

#define ITEM_ntotal(item)                                                      \
  (sizeof(struct _stritem) + (item)->nkey + 1 + (item)->nsuffix +              \
   (item)->nbytes + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))
...
struct stats {
  ...
  uint64_t time_in_listen_disabled_us; /* elapsed time in microseconds while
                                          server unable to process new
                                          connections */
...
struct settings {
  ...
  uint64_t
      oldest_cas; /* ignore existing items with CAS values lower than this */
  int evict_to_free;
  ...
  bool
      inline_ascii_response; /* pre-format the VALUE line for ASCII responses */
  ...
  bool
      drop_privileges; /* Whether or not to drop unnecessary process privileges
                        */
...
typedef struct _stritem {
  ...
  uint8_t slabs_clsid;
      /* which slab class we're in */ // 该item 属于那个slab class
...
typedef struct {
  ...
  uint8_t it_flags; /* ITEM_* above */ /* 标记了该ITEM的状态，非常重要 */
...
enum delta_result_type add_delta(conn *c, const char *key, const size_t nkey,
                                 const int incr, const int64_t delta, char *buf,
                                 uint64_t *cas);
