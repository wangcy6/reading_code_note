...
void assoc_init(const int hashtable_init) {
  if (hashtable_init) {
    hashpower = hashtable_init; //默认大小为16
  }
  // 申请哈希表空间
...
item *assoc_find(const char *key, const size_t nkey, const uint32_t hv) {
  ...
  // https://sq.163yun.com/blog/article/197772233171603456
  if (expanding &&
...
static item **_hashitem_before(const char *key, const size_t nkey,
  ...
  if (expanding &&
    ...
    pos = &primary_hashtable[hv &
                             hashmask(hashpower)]; // 找到哈希表中对应的桶位置
  }
 ...
 * this */
// 插入 item，hv 是哈希值
// https://cloud.tencent.com/developer/article/1147124
...
void assoc_delete(const char *key, const size_t nkey, const uint32_t hv) {
  item **before = _hashitem_before(
      key, nkey, hv); // 找到要删除 item 的前驱节点的 h_next 成员地址 
