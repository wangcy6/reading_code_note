...
ST_HIDDEN int _st_select_fd_getlimit(void) { return FD_SETSIZE; }

static _st_eventsys_t _st_select_eventsys = {"select",
                                             ST_EVENTSYS_SELECT,
                                             _st_select_init,
                                             _st_select_dispatch,
...
ST_HIDDEN int _st_poll_init(void) {
  ...
  _ST_POLLFDS =
      (struct pollfd *)malloc(ST_MIN_POLLFDS_SIZE * sizeof(struct pollfd));
  if (!_ST_POLLFDS) {
    free(_st_poll_data);
    _st_poll_data = NULL;
    return -1;
...
/* ARGSUSED */
ST_HIDDEN int _st_poll_fd_new(int osfd) { return 0; }
...
static _st_eventsys_t _st_poll_eventsys = {"poll",
                                           ST_EVENTSYS_POLL,
                                           _st_poll_init,
                                           _st_poll_dispatch,
...
ST_HIDDEN int _st_kq_init(void) {
  ...
  _st_kq_data->fd_data =
      (_kq_fd_data_t *)calloc(_st_kq_data->fd_data_size, sizeof(_kq_fd_data_t));
...
ST_HIDDEN int _st_kq_fd_data_expand(int maxfd) {
  ...
  ptr =
      (_kq_fd_data_t *)realloc(_st_kq_data->fd_data, n * sizeof(_kq_fd_data_t));
  if (!ptr)
    return -1;
...
ST_HIDDEN int _st_kq_addlist_expand(int avail) {
  ...
  ptr =
      (struct kevent *)realloc(_st_kq_data->addlist, n * sizeof(struct kevent));
  if (!ptr)
    return -1;
   ...
   */
  ptr =
      (struct kevent *)realloc(_st_kq_data->evtlist, n * sizeof(struct kevent));
  if (ptr) {
...
static _st_eventsys_t _st_kq_eventsys = {
    "kqueue",        ST_EVENTSYS_ALT,    _st_kq_init,
    _st_kq_dispatch, _st_kq_pollset_add, _st_kq_pollset_del,
    _st_kq_fd_new,   _st_kq_fd_close,    _st_kq_fd_getlimit};
#endif /* MD_HAVE_KQUEUE */
...
ST_HIDDEN int _st_epoll_init(void) {
  ...
  fdlim = st_getfdlimit();
  _st_epoll_data->fd_hint = fdlim;
  //   printf("set max fd %d \n",fdlim);
  //(fdlim > 0 && fdlim < ST_EPOLL_EVTLIST_SIZE) ?
  ...
  _st_epoll_data->fd_data = (_epoll_fd_data_t *)calloc(
  ...
  _st_epoll_data->evtlist = (struct epoll_event *)malloc(
      _st_epoll_data->evtlist_size * sizeof(struct epoll_event));
...
ST_HIDDEN void _st_epoll_dispatch(void) {
  ...
  // https://cloud.tencent.com/developer/article/1197338
  /* Check for I/O operations */
...
static _st_eventsys_t _st_epoll_eventsys = {"epoll",
                                            ST_EVENTSYS_ALT,
                                            _st_epoll_init,
                                            _st_epoll_dispatch,
...
int st_get_eventsys(void) { return _st_eventsys ? _st_eventsys->val : -1; }
