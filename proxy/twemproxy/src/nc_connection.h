...
struct conn {
    ...
    void                *owner;          /* connection owner - server_pool  每个连接属于一个server/ server  采用那个redis连接*/
    ...
    struct msg_tqh      imsg_q;          /* incoming request Q  需要往这个连接中写的msg push到这个消息队列*/
    struct msg_tqh      omsg_q;          /* outstanding request Q *///从连接上读到的数据往rmsg指向的msg里面填
   ...
   //typedef void (*conn_msgq_t)(struct context *, struct conn *, struct msg *);
    ...
    unsigned            client:1;        /* client? or server? */ //连接属于proxy和client之间时，client为1，连接属于proxy和后端server之间时
