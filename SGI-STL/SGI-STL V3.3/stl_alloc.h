...
public:
  ...
  static void *reallocate(void *__p, size_t __old_sz, size_t __new_sz) {
    ...
    char *__result = (char *)_Alloc::reallocate(
        __real_p, __old_sz + (int)_S_extra, __new_sz + (int)_S_extra);
...
// https://zhuanlan.zhihu.com/p/51918715
/**
 *
原本上，stl对于内存管理想必应该有惊人之语，应该一举解决大C++的整个内存管理上的心酸苦楚。
不料其格局也太狭隘，费大力气只是为了容器内存管理的一亩三分地这一点小地方，
真是让人大跌眼镜。这也罢了，内存管理这么运行时动态行为相关的操作，
居然当成是容器的模板参数出现，作为容器静态类型的一部分，
同时，容器还要兼容无状态以及有状态的allocator，无状态时，容器就不要包含allocator指针；
有状态时，就要充分表现多态分配器的能力。可以，这很零惩罚。这样

作者：恒虚之境
链接：https://www.zhihu.com/question/274802525/answer/476924532
来源：知乎
...
#ifdef __STL_THREADS
template <bool __threads, int __inst>
_STL_mutex_lock __default_alloc_template<
    __threads, __inst>::_S_node_allocator_lock __STL_MUTEX_INITIALIZER;
#endif
